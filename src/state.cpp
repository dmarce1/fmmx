...
state con_to_characteristic(const state& U_, const state& dU_, integer dir) {
	auto U = U_;
	auto dU = dU_;
	...
	const real p = pressure_eos(rho, (U[egi] - rho * (u * u + v * v + w * w) / real(2)) / rho);
	...
	const real dp = (fgamma - real(1)) * (dU[egi] - rho * (u * du + v * dv + w * dw) - drho * (u * u + v * v + w * w) / real(2));
	state dC(HYDRO_NF);
	dC[0] = drho - dp / (c * c);
	dC[1] = dv;
	dC[2] = dw;
	dC[3] = du + dp / (rho * c);
	dC[4] = du - dp / (rho * c);
	dC[5] = dU[epi] / rho - U[epi] / (rho * rho) * drho;
	return dC;
}

state characteristic_to_con(const state& U_, const state& dC, integer dir) {
	auto U = U_;
	...
	dU[5] = rho * dC[5] + dU[d0i] * U[epi] / U[d0i];
	return dU;
}
...
state flux(const state& u, integer dir, real phi) {
...
//	return roe(ur, ul, dir);
	...
	//const real phi = (ur[epi]/ur[d0i] + ul[epi]/ul[d0i])/real(2);
	const real phi = (phi_r + phi_l)/real(2);
	state F = (flux(ur, dir, phi) + flux(ul, dir, phi) - a * (ur - ul)) / real(2);
#ifdef TOTAL_ENERGY
	F[egi] -= a * (ur[d0i] - ul[d0i])*phi / real(2);
#endif
	return F;
}
