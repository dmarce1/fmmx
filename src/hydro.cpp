			...
			for (integer kk = 1; kk != HYDRO_NX; ++kk) {
				...
				for (integer g1 = 0; g1 != NGF; ++g1) {
					for (integer g2 = 0; g2 != NGF; ++g2) {
						...
						const real phi_rx = value_at(gphi[rk][i], -real(1), x1, x2);
						const real phi_ry = value_at(gphi[rk][i], x1, -real(1), x2);
						const real phi_rz = value_at(gphi[rk][i], x1, x2, -real(1));
						const real phi_lx = value_at(gphi[rk][i - di], +real(1), x1, x2);
						const real phi_ly = value_at(gphi[rk][i - dj], x1, +real(1), x2);
						const real phi_lz = value_at(gphi[rk][i - dk], x1, x2, +real(1));
					...
					for (integer gx = 0; gx != NGC; ++gx) {
						for (integer gy = 0; gy != NGC; ++gy) {
							for (integer gz = 0; gz != NGC; ++gz) {
								...
								const real phi = value_at(gphi[rk][i], x, y, z);
						...
						for (integer kk = kp; kk < kp + 2; ++kk) {
							...
							for (integer g1 = 0; g1 != NGF; ++g1) {
								for (integer g2 = 0; g2 != NGF; ++g2) {
									...
									const real phi_rx = value_at(gphi[rk][i], -real(1), gfpt[g1], gfpt[g2]);
									const real phi_lx = value_at(gphi[rk][i - di], +real(1), gfpt[g1], gfpt[g2]);
									state fx = Riemann_flux(urx, ulx, 0, phi_rx, phi_lx);
									for (integer l = 0; l != HYDRO_P; ++l) {
										for (integer m = 0; m != HYDRO_P - l; ++m) {
											for (integer n = 0; n != HYDRO_P - l - m; ++n) {
						...
						for (integer kk = kp; kk < kp + 2; ++kk) {
							...
							for (integer g1 = 0; g1 != NGF; ++g1) {
								for (integer g2 = 0; g2 != NGF; ++g2) {
									...
									const real phi_ry = value_at(gphi[rk][i], gfpt[g1], -real(1), gfpt[g2]);
									const real phi_ly = value_at(gphi[rk][i - dj], gfpt[g1], +real(1), gfpt[g2]);
									state fy = Riemann_flux(ury, uly, 1, phi_ry, phi_ly);
									for (integer l = 0; l != HYDRO_P; ++l) {
										for (integer m = 0; m != HYDRO_P - l; ++m) {
											for (integer n = 0; n != HYDRO_P - l - m; ++n) {
						...
						for (integer jj = jp; jj < jp + 2; ++jj) {
							...
							for (integer g1 = 0; g1 != NGF; ++g1) {
								for (integer g2 = 0; g2 != NGF; ++g2) {
									...
									const real phi_rz = value_at(gphi[rk][i], gfpt[g1], gfpt[g2], -real(1));
									const real phi_lz = value_at(gphi[rk][i - dk], gfpt[g1], gfpt[g2], +real(1));
									state fz = Riemann_flux(urz, ulz, 2, phi_rz, phi_lz);
									for (integer l = 0; l != HYDRO_P; ++l) {
										for (integer m = 0; m != HYDRO_P - l; ++m) {
											for (integer n = 0; n != HYDRO_P - l - m; ++n) {
...
void hydro::enforce_physical_boundaries(integer rk, integer dir) {
	...
	for (integer j = 0; j != HYDRO_NX; ++j) {
		for (integer k = 0; k != HYDRO_NX; ++k) {
			...
			if (dir == 0) {
				iii = gindex(0, j, k);
				U[rk][iii][0] = U[rk][gindex(1, j, k)][0];
			} else if (dir == 1) {
				iii = gindex(HYDRO_NX - 1, j, k);
				U[rk][iii][0] = U[rk][gindex(HYDRO_NX - 2, j, k)][0];
			} else if (dir == 2) {
				iii = gindex(j, 0, k);
				U[rk][iii][0] = U[rk][gindex(j, 1, k)][0];
			} else if (dir == 3) {
				iii = gindex(j, HYDRO_NX - 1, k);
				U[rk][iii][0] = U[rk][gindex(j, HYDRO_NX - 2, k)][0];
			} else if (dir == 4) {
				iii = gindex(j, k, 0);
				U[rk][iii][0] = U[rk][gindex(j, k, 1)][0];
			} else if (dir == 5) {
				iii = gindex(j, k, HYDRO_NX - 1);
				U[rk][iii][0] = U[rk][gindex(j, k, HYDRO_NX - 2)][0];
			}
			...
			for (integer n = 0; n != HYDRO_P - l - m; ++n) {
				for (integer j = 0; j != HYDRO_NX; ++j) {
					for (integer k = 0; k != HYDRO_NX; ++k) {
						const integer p = pindex(l, m, n);
						if (dir == 0) {
							gphi[rk][gindex(0, j, k)][p] = gphi[rk][gindex(1, j, k)][p];
						} else if (dir == 1) {
							gphi[rk][gindex(HYDRO_NX - 1, j, k)][p] = gphi[rk][gindex(HYDRO_NX - 2, j, k)][p];
						} else if (dir == 2) {
							gphi[rk][gindex(j, 0, k)][p] = gphi[rk][gindex(j, 1, k)][p];
						} else if (dir == 3) {
							gphi[rk][gindex(j, HYDRO_NX - 1, k)][p] = gphi[rk][gindex(j, HYDRO_NX - 2, k)][p];
						} else if (dir == 4) {
							gphi[rk][gindex(j, k, 0)][p] = gphi[rk][gindex(j, k, 1)][p];
						} else if (dir == 5) {
							gphi[rk][gindex(j, k, HYDRO_NX - 1)][p] = gphi[rk][gindex(j, k, HYDRO_NX - 2)][p];
						}
					}
				}
			}
...
std::vector<real> hydro::pack_phi_boundary(integer rk, integer d) const {
	...
	const integer yub = dir_y[d] == -1 ? 2 : HYDRO_NX - 1;
	const integer zub = dir_z[d] == -1 ? 2 : HYDRO_NX - 1;
	std::vector<real> a(HYDRO_PPP * (xub - xlb) * (yub - ylb) * (zub - zlb));
	auto iter = a.begin();
	for (integer i = xlb; i < xub; ++i) {
		for (integer j = ylb; j < yub; ++j) {
			for (integer k = zlb; k < zub; ++k) {
				...
				for (integer L = 0; L != FMM_P; ++L) {
					for (integer M = -L; M <= L; ++M) {
						...
						for (auto iter = std::begin(conv); iter != std::end(conv); ++iter) {
							integer ppp = pindex(iter->l, iter->m, iter->n);
							const auto u = U[rk][gindex(i, j, k)][ppp];
							real rho0 = u[d0i];
							if (ppp == 0) {
								rho0 -= rho_floor;
							}
							src[yi] -= iter->c * rho0 * std::pow(dx / real(2), real(L) + NDIM) * (M < 0 ? -real(1) : +real(1));
						}
					}
				}
			}
...
void hydro::set_gravity_sources(const std::vector<real>& src, integer rk) {
	...
	for (integer i1 = 1; i1 != HYDRO_NX - 1; ++i1) {
		for (integer j1 = 1; j1 != HYDRO_NX - 1; ++j1) {
			for (integer k1 = 1; k1 != HYDRO_NX - 1; ++k1) {
				const integer ii = gindex(i1, j1, k1);

				for (integer g2x = 0; g2x < HYDRO_P; ++g2x) {
					for (integer g2y = 0; g2y < HYDRO_P; ++g2y) {
						for (integer g2z = 0; g2z < HYDRO_P; ++g2z) {
							real rho0 = value_at(U[rk][ii], gpt[g2x], gpt[g2y], gpt[g2z])[d0i];
							rho0 *= dx * dx * dx * gwt[g2x] * gwt[g2y] * gwt[g2z] / real(8);
							for (integer l = 0; l != HYDRO_P; ++l) {
								for (integer m = 0; m != HYDRO_P - l; ++m) {
									for (integer n = 0; n != HYDRO_P - l - m; ++n) {
										const integer ppp = pindex(l, m, n);

										for (integer g1x = 0; g1x < HYDRO_P; ++g1x) {
											for (integer g1y = 0; g1y < HYDRO_P; ++g1y) {
												for (integer g1z = 0; g1z < HYDRO_P; ++g1z) {
													const real px = LegendreP(l, gpt[g1x]) * gwt[g1x] * real(2 * l + 1) / real(2);
													const real py = LegendreP(m, gpt[g1y]) * gwt[g1y] * real(2 * m + 1) / real(2);
													const real pz = LegendreP(n, gpt[g1z]) * gwt[g1z] * real(2 * n + 1) / real(2);
													const real wt = px * py * pz;

													const real delta_x = ((gpt[g2x] - gpt[g1x]) / real(2)) * dx;
													const real delta_y = ((gpt[g2y] - gpt[g1y]) / real(2)) * dx;
													const real delta_z = ((gpt[g2z] - gpt[g1z]) / real(2)) * dx;
													const real r = std::sqrt(delta_x * delta_x + delta_y * delta_y + delta_z * delta_z);
													if (r > real(0)) {
														const real rinv = real(1) / r;
														gphi[rk][ii][ppp] -= rho0 * rinv * wt;
													}
												}
											}
										}
	...
	for (integer iii = 0; iii != HYDRO_N3; ++iii) {
		for (integer l = 0; l != HYDRO_P; ++l) {
			for (integer m = 0; m != HYDRO_P - l; ++m) {
				for (integer n = 0; n != HYDRO_P - l - m; ++n) {
					integer ppp = pindex(l, m, n);
					U[rk][iii][ppp][poi] = real(0);
					for (integer g1x = 0; g1x < HYDRO_P; ++g1x) {
						for (integer g1y = 0; g1y < HYDRO_P; ++g1y) {
							for (integer g1z = 0; g1z < HYDRO_P; ++g1z) {
								const real px = LegendreP(l, gpt[g1x]) * gwt[g1x] * real(2 * l + 1) / real(2);
								...
								const real pz = LegendreP(n, gpt[g1z]) * gwt[g1z] * real(2 * n + 1) / real(2);
								const real wt = px * py * pz;
								const real phi = value_at(gphi[rk][iii], gpt[g1x], gpt[g1y], gpt[g1z]);
								const real rho = value_at(U[rk][iii], gpt[g1x], gpt[g1y], gpt[g1z])[d0i];
								U[rk][iii][ppp][poi] += phi * rho * wt;
							}
						}
					}
				}
